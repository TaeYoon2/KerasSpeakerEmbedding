...
this.onmessage = function(e){
    ...
    case 'exportRAW':
      ...
      break;
    case 'export16kMono':
      // export16kMonoSync(e.data.buffer, e.data.type);
...
function init(config){
  ...
  resampler = new Resampler(sampleRate, 16000, 1, 500*1024);
}

function record(inputBuffer){
    // var buffer = resampler.resampler(inputBuffer[0]);
...
//  var interleaved = mergeBuffers(recBuffers, recLength);
 ...
 var buffer = mergeBuffers(fullBuffers, fullLength);
  // var interleaved = mergeBuffers(fullBuffers, fullLength);
...
function export16kMonoSync(recbuffer, type) {
	fullBuffers.push(recbuffer);
	fullLength += recbuffer.length;
    //var buffer = mergeBuffers(recbuffer, bufferleng);
    var samples = resampler.resampler(recbuffer);
